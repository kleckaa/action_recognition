...
def train_model(args, log_dir):
    ...
    log_folder = log_dir + os.path.join(args.logdir, arch_name)
    ...
    elif args.lr_scheduler == 'cosine2':
        scheduler = lr_scheduler.CosineAnnealingLR(optimizer, args.epochs, eta_min=1e-5)
    ...
    elif args.lr_scheduler == 'restart_2':
        scheduler = lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=20, T_mult=1, eta_min=1e-8)
    elif args.lr_scheduler == 'CyclicLR_1':
        scheduler = lr_scheduler.CyclicLR(optimizer, base_lr=1e-8, max_lr=0.001, mode='triangular', step_size_up=15)
    elif args.lr_scheduler == 'CyclicLR_2':
        scheduler = lr_scheduler.CyclicLR(optimizer, base_lr=1e-8, max_lr=0.001, mode='triangular2', step_size_up=15)
    elif args.lr_scheduler == 'CyclicLR_3':
        scheduler = lr_scheduler.CyclicLR(optimizer, base_lr=1e-8, max_lr=0.001, mode='exp_range', step_size_up=15)
    elif args.lr_scheduler == 'CyclicLR_4':
        scheduler = lr_scheduler.CyclicLR(optimizer, base_lr=1e-8, max_lr=0.001, mode='triangular', step_size_up=15)
    elif args.lr_scheduler == 'CyclicLR_5':
        scheduler = lr_scheduler.CyclicLR(optimizer, base_lr=1e-8, max_lr=0.001, mode='triangular2', step_size_up=15)
    elif args.lr_scheduler == 'CyclicLR_6':
        scheduler = lr_scheduler.CyclicLR(optimizer, base_lr=1e-8, max_lr=0.001, mode='exp_range', step_size_up=15)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # with open('/storage/plzen1/home/kleckaa/i3d_models/rgb/' + args.lr_scheduler +'.txt','a') as f:
        # f.write(str(scheduler.optimizer.param_groups[0]['lr']))
        # f.write('\n')
        ...
        if args.rank == 0:
            ...
            if val_losses < best_val:
                # print(val_losses)
                if not os.path.exists(log_folder + '/best_val/'):
                    os.makedirs(log_folder + '/best_val/')
                best_val = val_losses
                save_checkpoint(save_dict, True, filepath=log_folder + '/best_val/')
